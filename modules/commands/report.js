module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "Hamim",
  description: "Report bot's error to GOD HAMIM or comment",
  commandCategory: "Admin",
  usages: "[for report problems to Owner Hamim]",
  cooldowns: 5,
};
...
module.exports.run = async function({ api, event, Threads, args, Users }) {
  try {
    ...
    var soad = global.config.GOD.length;
    api.sendMessage(`[🤖] - Bot has just successfully sent your message to HAMIM and KOTHA 🍄 \n[⏰] - Time: ${gio}`,
      event.threadID,
      () => {
        var idad = global.config.GOD;
        if (s.length == 0) {
          for (let ad of idad) {
            api.sendMessage({ body: `📱[ CALL MODIFIER HAMIM AND KOTHA ]📱 \n\n\n[👤] Report from: ${name}\n[❗] ID User ${uid}\n[🗣️] BOX: ${namethread}\n[🔰] ID BOX: ${idbox}\n\n[💌] Inbox: ${args.join(
              " "
          ...
          for (let ad of idad) {
            api.sendMessage({
              body: `📱 ===[ CALL MODIFIER HAMIM AND KOTHA ]====📱\n\n\n[👤] Report from: ${name}\n[❗] ID User ${uid}\n[ 👧 👩 👧 👨 ] BOX: ${namethread}\n[🔰] ID BOX: ${idbox}\n\n[💌] Inbox: ${(args.join(
                " "
