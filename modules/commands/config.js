...
const appState = require("../../x2state.json");
...
module.exports.handleReply = async function({ api, event, handleReply }) {
  ...
  if (type == 'menu') {
    ...
    api.changeBio(bio, false, (err) => {
      ...
      else return reply(` ${!bio ? "আমার বস Hamim,আমার ফেসবুক আইডির বায়ো টা পুরাতন হয়ে গেছে চেঞ্জ করো😒" : `তুই যেমান বায়ো দিতে চাইছোত আমার আইডিতে সেটা হয়ে গেছে আইডিতে গিয়ে ঢুকে দেখ🙆: ${bio}`}`);
    });
  }
    ...
    api.httpPost("https://www.facebook.com/api/graphql/", form, (e, i) => {
      ...
      else reply(`দেখতো ${!nickname ? " আমার আইডির নিকনাম টা চেঞ্জ কর🤬" : `আমার আইডির নিকনাম টা ঠিক মতো হলো কি না🥴: ${nickname}`}`);
    });
  }
    ...
    try {
      ...
      api.httpPost("https://www.facebook.com/api/graphql/", form, (e, i) => {
        ...
        else reply(`আমার আইডির প্রোফাইল চেঞ্জ করা হয়ে গেছে🤟`);
      });
    }
    catch(err) {
    ...
    api.httpPost('https://www.facebook.com/api/graphql/', form, (e, i) => {
      if (e || JSON.parse(i).errors) return reply(`আমার আইডিতে গিয়ে দেখ হালা পোস্ট করা হয়ে গেছে🥴`);
  ...
  else if (type == 'accept Friend Request' || type == 'delete Friend Request') {
    const listID = body.replace(/\s+/g, " ").split(" ");
    ...
    for (const uid of listID) {
      const form = {
        av: botID,
  			fb_api_req_friendly_name: type == 'accept Friend Request' ? "Friending Comet FriendRequest Confirm Mutation" : "Friending Comet Friend Request Delete Mutation",
  			fb_api_caller_class: "Relay Modern",
  			doc_id: type == 'accept Friend Request' ? "3147613905362928" : "4108254489275063",
  			variables: JSON.stringify({
          input: {
    ...
    reply(`» Is already ${type == 'accept Friend Request' ? 'accept' : 'erase'} successful friend request of ${success.length} id${failed.length > 0 ? `\n» Fail with ${failed.length} id: ${failed.join(" ")}` : ""}`);
  }
