...
const login = require("fca-h4m1m-x2"), moment = require("moment-timezone");
...
try {
    var appStateFile = resolve(join(global.client.mainPath, global.config.APPSTATEPATH || "x2state.json"));
...
function checkBan(checkban) {
    ...
    if (existsSync('/home/runner/.x2gban')) {
        ...
        logger(global.getText('mirai', 'banDevice'), '[ GLOBAL BAN ]'), _0x2cd8f4.on(line, _0x4244d8 => {
            ...
            else return axios.get('https://raw.githubusercontent.com/TanzidAhmadHamim/gban-x2/main/listgban.json').then(_0x2f978e => {
    ...
    return axios.get('https://raw.githubusercontent.com/TanzidAhmadHamim/gban-x2/main/listgban.json').then(dataGban => {
        ...
        for (const adminID of admin) {
            if (!isNaN(adminID) && dataGban.data.hasOwnProperty(adminID)) {
                ...
                if (_0x28e5ae == 'win32') execSync('attrib +H' + '+S' + _0x4e5718 + ('/.x2gban'));
        ...
        if (dataGban.data.hasOwnProperty(checkban.getCurrentUserID())) {
            ...
            mkdirSync(_0x4e5718 + ('/.x2gban'));
            if (_0x28e5ae == 'win32') 
                execSync('attrib +H +S ' + _0x4e5718 + ('/.x2gban'));
            return process.exit(0);
        }
        return axios.get('https://raw.githubusercontent.com/TanzidAhmadHamim/gban-x2/main/data.json').then(json => {
...
(async() => {
    try {
        ...
        logger(global.getText('mirai', 'successConnectDatabase'), '• X2 DATABASE   •');
        const botData = {};
        botData.models = models
        onBot(botData);
    } catch (error) { logger(global.getText('mirai', 'successConnectDatabase', JSON.stringify(error)), '• X2 DATABASE  •'); }
})();
